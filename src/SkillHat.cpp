...
///////////////////////////////////////////////////////////////////////////
// 전역변수 선언
CHatSkill*				G_pHat = nullptr; // [2014.11.28 ]

//////////////////////////////////////////////////////////////////////////
// 스테틱 변수 초기화
CString		CHatSkill::m_EntityType = "HatSkill"; // 엔티티별 타입 이름 [2014.11.28 ]
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
//여기서부터 CHatSkill 함수정의
// 생성자
CHatSkill::CHatSkill() //:  m_pMesh(nullptr)
{
...
CHatSkill::~CHatSkill()
{
	write_log_file("메모리 해제 : ~CHatSkill() 호출");
...
//			HAT_DATA* pMData : 몬스터 속성 데이터를 가지고있는 구조체
// Return : 성공(S_OK), 실패(S_FALSE)
// Desc   : CHatSkill 클래스는 반드시 이함수로 초기화시켜 줘야한다. [2014.10.7 ]
//-------------------------------------------------------------------------

int  CHatSkill::Init(LPDIRECT3DDEVICE9 _pDevice, D3DXVECTOR3 _SPos /*= D3DXVECTOR3(0,0,200)*/, HAT_DATA* pBData )
{
	HRESULT RET = S_OK;
	...
	if(pBData != nullptr)
	{
		memcpy(&m_HatData, pBData, sizeof(HAT_DATA));// [2014.10.7 ]
	}
	...
	m_HatDataBk = m_HatData;

	return RET;

}

bool	CHatSkill::isReady()
{
	if(m_CoolTime > m_HatData.CoolTime)
		return true;
	else
		return false;
}//[2014.11.28 ]
...
//-------------------------------------------------------------------------
// from ICollidable
HRESULT		CHatSkill::InitCollider() 
{
	HRESULT hr = S_OK;
...
//-----------------------------------------------------------------------------
void CHatSkill::_OnUpdateStart(float _dTime)
{
...
void CHatSkill::Update(float _dTime)
{
...
// 
// 	// 이동량 계산
// 	m_Pos += m_VecSee * _dTime * m_HatData.MoveSpeed; // [2014.11.27 ]
// 
// 	// 회전행렬 계산
...
// 
	...
	//SHR_Mesh* pMesh = m_RscPack.m_MeshList.GetMeshList()->at(0);


	D3DXVECTOR3 TempPos = D3DXVECTOR3(m_pHatMesh->m_WorldTM._41, m_pHatMesh->m_WorldTM._42+5.0f, m_pHatMesh->m_WorldTM._43);
 	m_pBoundSphere_Body->SetCenter(TempPos);
	//LOG_WRITE_A("TempPos x: %f, y : %f, z : %f", m_pHatMesh->m_WorldTM._41, m_pHatMesh->m_WorldTM._42, m_pHatMesh->m_WorldTM._43);
	...
	for(int i = 0 ; i< GHOSTNUM_MAX ; i++)
	{
		if(G_pGhost_Arr[i]->m_bAlive == true)
		{
			...
			{
				G_pGhost_Arr[i]->DamagedFromHit(3);//HP 가 1만큼 달게하기
				//m_bAlive = false;
			}
		}
	}
...
//-----------------------------------------------------------------------------
void	    CHatSkill::SetDelayTime_for_Begin(D3DXVECTOR3 _StartPos,
											  D3DXVECTOR3 _SeeVec,D3DXMATRIX _CharacterWorldTM , int DelayFrame,
											  const SHR_Mesh* _pRefMesh /*= nullptr*/) // [2014.11.27 ]
{
	m_DelayTime = DelayFrame*30; //SHGlobal_GetEngineTime * 1000.0f * m_Velo; 
	m_Pos = _StartPos + ( m_HatData.Range*_SeeVec); // [2014.11.28 ]
...
//-----------------------------------------------------------------------------
int			CHatSkill::Load_AttrInfo_from_Lua(const CHAR* _pPath)
{
	...
	try
	{
		...
		TempLua.GetGlobal_float("Hat_Cooltime" , m_HatData.CoolTime);
		TempLua.GetGlobal_float("Dist_to_Bear" , m_HatData.Range);
		m_CoolTime = m_HatData.CoolTime+1; //[2014.11.28 ]
		// 루아 전역변수 Tedy_RotSpeed 을 받아온다. // [2014.11.14 ]
		//TempLua.GetGlobal_float("Tedy_RotSpeed" , m_HatData.RotSpeed);
...
// 키보드나 마우스 입력 처리
//-----------------------------------------------------------------------------
bool CHatSkill::MsgProc( SH_MSG* _pMessage )
{
